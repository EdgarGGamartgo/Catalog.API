...
namespace Catalog.Fixtures
{
    public class LoadDataAttribute : DataAttribute
    {
        ...
        public override IEnumerable<object[]> GetData(MethodInfo testMethod)
        {
         ...
         //   if (!File.Exists(path)) throw new ArgumentException($"File not found: {path} {File.Exists(path)}");

        //    var fileData = File.ReadAllText(_fileName);
        var fileData = File.ReadAllText("C:/aspnet/Catalog.API/tests/Catalog.Fixtures/record-data.json");
